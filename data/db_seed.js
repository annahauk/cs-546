...
async function main() {
	...
	try {
		...
		try {
			console.log(
				"Attempting to create a user with a duplicate username (johnDoe)..."
			);
		...
		const updatedTags = await updateUserTags(user1._id, [
			"JavaScript",
			"Node.js"
		]);
		...
		const filteredPosts = await grabfilteredPosts({
