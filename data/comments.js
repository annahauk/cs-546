...
// helper imports
import {stringVal, exists, validLength, idVal} from '../helpers.js'
...
async function createComment(content, postId, ownerId) {
  ...
  const post = await postCollection.findOneAndUpdate({_id: postIdObj}, {$push: {comments:newComment}});
...
async function getCommentById(commentId) {
  ...
  idVal(commentId);
  const postCollection = await projectPosts();
  const foundComment = await postCollection.findOne({'comments._id': new Object(commentId)}, {projection: {_id: 0, 'comments.$': 1}});
...
async function removeComment(commentId) {
  ...
  const postCollection = await projectPosts();
  const foundPost = await postCollection.findOne({'comments._id': commentIdObj});
  if (!foundPost) throw `No post found with that comment id: ${commentId}`
  const updateInfo = await postCollection.updateOne({_id: foundPost._id}, {$pull: {comments:{commentIdObj}}});
