...
function nameVal(name) {
	...
	if (nameverif.length === 2) {
		if (nameverif[0].length < 3 || nameverif[1].length < 3)
			throw "First name and last name must have at least 3 characters";
	}
	return name;
}
...
function validateRuntime(runtime) {
	...
	if (
		Number.isInteger(parseFloat(hours)) === false ||
		Number.isInteger(parseFloat(minutes)) === false
	) {
